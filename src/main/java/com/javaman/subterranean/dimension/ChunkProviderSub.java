...
public class ChunkProviderSub implements IChunkProvider
{
    ...
    public ChunkProviderSub(World worldIn, boolean p_i45637_2_, long p_i45637_3_)
    {
        this.field_177467_w = new WorldGenMinable(Blocks.quartz_ore.getDefaultState(), 14, BlockHelper.forBlock(ModBlocks.SUBTERRANEAN_BLOCKS));
                    ...
                    for (int l1 = 0; l1 < 8; ++l1)
                    {
                        ...
                        for (int i2 = 0; i2 < 4; ++i2)
                        {
                            ...
                            for (int j2 = 0; j2 < 4; ++j2)
                            {
                                ...
                                if (d15 > 0.0D)
                                {
                                    iblockstate = ModBlocks.SUBTERRANEAN_BLOCKS.getDefaultState();
                                }
    ...
    public void func_180516_b(int p_180516_1_, int p_180516_2_, ChunkPrimer p_180516_3_)
    {
        ...
        for (int k = 0; k < 16; ++k)
        {
            for (int l = 0; l < 16; ++l)
            {
                ...
                int j1 = -1;
                IBlockState iblockstate = ModBlocks.SUBTERRANEAN_BLOCKS.getDefaultState();
                IBlockState iblockstate1 = ModBlocks.SUBTERRANEAN_BLOCKS.getDefaultState();
                ...
                for (int k1 = 127; k1 >= 0; --k1)
                {
                    if (k1 < 127 - this.hellRNG.nextInt(5) && k1 > this.hellRNG.nextInt(5))
                    {
                        ...
                        {
                            if (iblockstate2.getBlock() == ModBlocks.SUBTERRANEAN_BLOCKS)
                            {
                                if (j1 == -1)
                                {
                                    if (i1 <= 0)
                                    {
                                        iblockstate = null;
                                        iblockstate1 = ModBlocks.SUBTERRANEAN_BLOCKS.getDefaultState();
                                    }
    ...
    public void populate(IChunkProvider p_73153_1_, int p_73153_2_, int p_73153_3_)
    {
       ...
       */ doGen = TerrainGen.decorate(worldObj, hellRNG, blockpos, net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.BIG_SHROOM);
       ...
       doGen = TerrainGen.decorate(worldObj, hellRNG, blockpos, net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.LILYPAD);
       if (doGen && this.hellRNG.nextBoolean())
       {
           this.field_177471_z.generate(this.worldObj, this.hellRNG, blockpos.add(this.hellRNG.nextInt(16) + 8, this.hellRNG.nextInt(128), this.hellRNG.nextInt(16) + 8));
       }
